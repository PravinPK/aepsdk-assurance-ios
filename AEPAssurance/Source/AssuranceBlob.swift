...
enum AssuranceBlob {
    ...
    static func sendBlob(_ blob: Data, forSession session: AssuranceSession, contentType: String, callback : @escaping (String?) -> Void) {
        ...
        components.host = String.init(format: HOST_FORMAT, session.state.environment.urlFormat)
        components.path = UPLOAD_PATH
        components.queryItems = [
            URLQueryItem(name: QUERY_VALIDATION_SESSION_ID, value: session.state.sessionId)
        ]
